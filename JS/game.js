...
function loadItems(category) {
  ...
  list.forEach(item => {
    ...
    div.addEventListener('click', () => {
      // Блокируем ручную смену, если категория заблокирована (разрешаем очистку слотом empty)
      if (item.id !== `${category}_empty` && lockedCategories[category]) return;
      // Удаляем предыдущий основной слой категории
      const oldMain = avatarCanvas.querySelector(`img[data-category="${category}"]`);
      if (oldMain) avatarCanvas.removeChild(oldMain);
      // Если был ранее применён pairedItem, снимаем его и разблокируем категорию
      if (pairedMap[category]) {
        const linkedCat = pairedMap[category];
        const el = avatarCanvas.querySelector(`img[data-category="${linkedCat}"]`);
        if (el) avatarCanvas.removeChild(el);
        lockedCategories[linkedCat] = false;
        delete pairedMap[category];
      }
      // Обновляем выделение
      inventoryBar.querySelectorAll('.inventory-item').forEach(el => el.classList.remove('selected'));
      div.classList.add('selected');
      // Обработка снятия через empty-слот
      if (item.id === `${category}_empty`) {
        saveAvatarConfig(getAvatarConfig());
        document.getElementById('color-bar').innerHTML = '';
        return;
      }
      // Применяем новый основной предмет
      ...
      if (item.pairedItem) {
        ...
        Object.keys(itemsList).forEach(catKey => {
          if ((itemsList[catKey] || []).some(it => it.id === item.pairedItem)) linkedCat = catKey;
        });
        if (linkedCat) {
          applyToAvatar(linkedCat, item.pairedItem, null, 'public');
          lockedCategories[linkedCat] = true;
          pairedMap[category] = linkedCat;
        }
      }
